...
## 2026-09-05 — Nuevo head `retorno_acumulado_dia`: la elección de params optimiza retorno acumulado en el tiempo, no promedio por trade

**Motivo:** el notebook de análisis exploratorio (`X5_analisis_exploratorio.ipynb`) mostró que `A` (ganancia mínima en USD para activar el primer SL ganador) tenía alto impacto en `retorno_usd` **promedio por operación cerrada**. Pero ese no es el objetivo real: un `A` alto puede ganar en retorno promedio por trade y perder en cuántas veces cierra (muchas posiciones nunca llegan al umbral), perdiendo en el acumulado — el clásico P (retorno promedio) vs P×Q (retorno promedio × frecuencia = lo que realmente se acumula en el tiempo). Tanto `_inferir_lgbm` (Optuna) como `_inferir_ftt` (gradient ascent) optimizaban únicamente el head `retorno` (por trade), sin ver este efecto.

**Problema de atribución detectado durante el diseño:** una orden puede abrirse bajo el tramo T (params fijos entre recálculos de soportes, `X5_RECALC_SOPORTES_CADA`) y cerrar varios tramos después — X4 evalúa el trailing/breakeven de **todas** las OA abiertas con el `A`/`B` **actual** en cada vela (`X4_backtester.py`, gestión de OA), no el vigente cuando cada una se abrió. Sumar `retorno_usd` de las OC agrupadas por tramo de cierre le daría crédito de más a ese tramo.

**Decisión — target por variación de equity, no por suma de trades cerrados:** `_construir_tramos` (`X5_macro_brain.py`) reconstruye los tramos por changepoints en `PARAM_STORE_COLS`, ordenando oc+periodico por `timestamp_oc`. El target `retorno_acumulado_dia` no suma `retorno_usd`: mide `Δ(pnl_cerrado_activo + pnl_flotante_activo)` entre el inicio de un tramo y el inicio del siguiente, por día. Como pasar de "flotante" a "cerrado" no cambia esa suma, cada tramo se lleva crédito exactamente por el movimiento de cuenta ocurrido mientras sus propios params estaban vigentes, sin importar cuándo abrió o cerró cada posición. Validado contra el store real de BTCUSD: 194 tramos, 17 (8.8%) sin ninguna OC — invisibles para el análisis por trade, visibles (y penalizados) acá.

**Descartado:** sumar `retorno_usd` de las OC agrupadas por tramo de cierre — es más simple pero tiene el sesgo de atribución cross-tramo descrito arriba.

**Head siempre LightGBM, incluso en fase FTT:** con datos reales, 5.426 OC colapsan en 194 tramos — un tramo agrupa en promedio ~5 días de trades. El n de tramos nunca se acerca a `X5_MIN_TRADES_FTT` (5.000), así que no se justifica meter 'acumulado' en el FT-Transformer. Se entrena aparte (`_entrenar_acumulado`, `{activo}_lgbm_acumulado.pkl` + features.json propio) y se llama siempre desde `_entrenar()`, independiente de si el resto del modelo ya es lgbm o ftt. Nuevo umbral `X5_MIN_TRAMOS_TRAIN = 30` en `config.py`.

**Inferencia y fallback:** `cargar_modelo_para_activo` retorna ahora `(tipo, bundle, acumulado)`. `inferir_con_contexto` optimiza SIEMPRE `acumulado` vía Optuna (model-agnostic — funciona igual si el resto del modelo es lgbm o ftt) cuando está disponible; si el activo todavía no junta `X5_MIN_TRAMOS_TRAIN` tramos, cae al comportamiento anterior (Optuna sobre `retorno` en fase lgbm, gradient ascent sobre `retorno` en fase ftt). Se extrajo `_optimizar_optuna` como helper reusable (antes vivía inline en `_inferir_lgbm`). `X4_backtester.py` (`_seleccionar_params_x5`) actualizado a la tupla de 3 elementos.

**Validación:** con modelos sintéticos de signo opuesto (uno prefiere `A` bajo, el otro `A` alto), se confirmó que `inferir_con_contexto` usa `acumulado` cuando está disponible y cae a `retorno` cuando no. La Sección 5.6/5.7 del notebook exploratorio replica el mismo cálculo de tramos (implementación propia, standalone — no importa de `X5_macro_brain.py`, son consumidores distintos) para comparar visualmente el ranking de cada parámetro en ambas lecturas; con el store real de BTCUSD, el parámetro de mayor efecto cambia de `N_EXP` (por trade) a `LAMBDA` (acumulado) — evidencia directa de que ambas preguntas no son intercambiables.

## 2026-09-21 — X0: ciclos independientes por activo (antes: un solo ciclo sincronizado para todos)

**Motivo:** el loop de `X0_data_supports.py` era un único "CICLO" que esperaba a que los 6 activos convergieran (`ProcessPoolExecutor` + `as_completed` bloqueante) antes de volver a descargar datos/X3/X2 y relanzar la búsqueda de soportes para todos. Un activo que converge rápido quedaba esperando a los más lentos en vez de aprovechar datos más frescos de inmediato. El monitor además imprimía una sola línea (`\r`) con los 6 activos separados por `|`, difícil de leer.

**Decisión — loop por activo, no por lote global:**
- `_ciclo_activo` (nuevo, reemplaza `buscar_soportes`/`_seleccionar_combos`): un hilo por activo, cada uno con su propio contador de ciclo (`ciclos_estado`). Al converger, ese activo re-descarga sus datos (H1+M1), recalcula X3, y arranca su siguiente ciclo de inmediato — sin esperar a los demás. Los 6 hilos comparten un único `ProcessPoolExecutor` (paralelismo real para el cómputo pesado del optimizador); las llamadas a MT5 se serializan con un `threading.Lock` (`mt5_lock`) porque la API no es thread-safe para llamadas concurrentes.
- `_monitor_log` (reemplaza `_monitor_tabla`): en vez de redibujar una línea con `\r`, imprime una línea nueva por activo cada vez que su estado cambia, tageada con su ciclo (`BTCUSD [c3] 12/40 FO=...`). El diseño previo evitaba deliberadamente el redraw multi-línea por desincronización en Windows si algo más escribía a stdout de por medio (ver comentario histórico en el código) — un log que solo agrega líneas no tiene ese problema, porque nunca sobrescribe.
- **X2 se desacopla del ciclo por activo** (`_x2_watchdog`, hilo propio con reintento cada 1h): su scoring normaliza cada activo contra el mínimo/máximo del universo completo (`VALORES`), así que no existe una versión "solo para un activo" sin cambiar el modelo de scoring. Ya trae guard de un día, así que el watchdog es casi siempre no-op.
- `N_MAX_MODELS` cambia de significado: antes seleccionaba qué combos correr ese ciclo global (top-delta); ahora es el `max_workers` del `ProcessPoolExecutor` compartido (tope de combos corriendo a la vez). Con 6 activos × 1 N cada uno (config actual), el comportamiento efectivo no cambia (6 = todos).
- `--ciclos N` pasa a contar ciclos por activo (cada hilo se detiene solo al llegar a N), no un contador global compartido.

**Descartado:** mantener el redraw multi-línea en vivo (revivía el problema de desincronización en Windows que motivó el diseño de una sola línea) y disparar X2 por activo (no tiene sentido matemático dado el scoring cross-sectional).

**Validado:** smoke test local (`--opcion 1 --ciclos 1`, sin MT5) — los 6 activos arrancaron su ciclo 0 en paralelo e imprimieron su diagnóstico de forma independiente; el watchdog de X2 corrió en paralelo sin bloquear a los activos.

## 2026-09-22 — X0: monitor vuelve a redraw multi-línea en vivo, esta vez con `rich.Live` (revierte parte de la decisión del 2026-09-21)

**Motivo:** Mauricio pidió que el monitor vuelva a mostrar una línea fija por combo que se **reemplaza** en el lugar (no una línea nueva por cambio de estado como hacía `_monitor_log`), con las fases del ciclo completo visibles ahí: calculando distancias, optimizador corriendo, convergencia, actualizando data hora/minuto, actualizando X2/X3. Antes de tocar código se le mostró el historial completo de este mismo patrón fallando 3 veces en el proyecto (`_monitor_tabla`: ANSI deshabilitado en cmd.exe → fix con `os.system('')`; luego desync por wrap de línea en terminales angostas → fix con `shutil.get_terminal_size()`; finalmente abandonado ayer por seguir desincronizándose). Eligió retomarlo igual, pero con `rich` en vez de ANSI a mano.

**Por qué esta vez es distinto (validado empíricamente, no solo en teoría):**
- `rich.live.Live` + `rich.console.Console` compartidos por todo el proceso principal: cualquier `console.print()` concurrente con el redraw se coordina automáticamente (Rich pausa el live, imprime la línea "normal" arriba, redibuja el bloque abajo) — no hay math de cursor a mano que desincronizar.
- El ancho de terminal (causa del segundo bug histórico) lo calcula y maneja Rich internamente en cada frame — no hay padding fijo que rompa con líneas largas o ventanas angostas.
- `redirect_stdout=True`/`redirect_stderr=True` (default de `Live`) es una red de seguridad adicional: incluso un `print()` suelto que se escape de la conversión manual (o de una librería de terceros, ej. `MetaTrader5`) queda coordinado igual, en vez de corromper el redraw.
- Validado con un test de concurrencia real (6 hilos con `print()` crudo intercalado + redraw cada 10ms) sin ninguna línea corrompida ni mezclada — el mecanismo serializa correctamente incluso sin pasar por `console.print`.

**Cambios:**
- `X0_data_supports.py`: nuevo `console = Console()` a nivel de módulo — reemplaza todos los `print()` que podían correr en paralelo con el loop (descargas, `_info_previa_combo`, `_resumen_combo`, banner de ciclo, `_log_error`, `json_act`, `obtener_ordenes_activas_mt5`). `_monitor_log` → `_live_monitor` (mismo polling de `estado_compartido`/`ciclos_estado`, pero llama `live.update(...)` en vez de `print()`). Nuevo `_construir_tabla_viva` arma un `rich.console.Group` de una `Text` por combo + una línea global de X2 (X2 sigue desacoplado del ciclo por activo — scoring cross-sectional, sin cambios ahí — pero ahora tiene su propia línea en vez de compartir la de un combo cualquiera). Nuevas fases en `estado_compartido`: `'actualizando data (hora)'`, `'actualizando data (minuto)'`, `'actualizando X3'` (seteadas desde `_ciclo_activo`, antes de cada llamada correspondiente); `'convergencia'` reemplaza al antiguo `f'listo {duracion}s'` final de `_procesar_valor_N`.
- **Información inicial solo en ciclo 0 por activo:** `_ciclo_activo` gatea tras `es_primer_ciclo = (ciclo == 0)` el banner `── valor: ciclo N ──`, el `verbose` de `descargar_datos`/`descargar_datos_minuto`/`actualizar_features` (X3), y las llamadas a `_info_previa_combo`/`_resumen_combo`. De ciclo 1 en adelante no se imprime ninguna línea nueva — todo el estado vive en la línea en vivo del combo. `descargar_datos`, `descargar_datos_minuto` (`X0_data_supports.py`) y `actualizar_features` (`X3_technical_features.py`) ganan parámetro `verbose: bool = True`; esta última también `console=None` opcional (usa `print` si no se pasa, para no acoplar X3 a `rich` cuando se llama fuera de X0).
- `_x2_watchdog` ahora corre el subprocess de X2 con `capture_output=True` (antes heredaba stdout directo) — un subprocess escribiendo directo a la consola es un proceso de SO distinto, invisible para la coordinación de `Live`, así que su output se captura y se imprime de una vez por `console.print` al terminar. Reporta su estado (`actualizando` / `ok` / `ERROR`) en un dict simple (`x2_estado`, mismo proceso, sin necesidad de `Manager`).
- Nueva dependencia: `rich` (instalado en `revenAI` en Mac; falta instalarlo también en el `revenAI` de Windows antes de correr X0 ahí — `pip install rich`).

**Descartado:** seguir con ANSI a mano ruteado por un coordinador propio (mismo tipo de bug que ya falló 3 veces, sin la robustez ya probada de una librería) y un archivo de estado aparte + `watch`/`Get-Content -Wait` en otra ventana (más simple y con cero riesgo de desync, pero exige una segunda ventana — Mauricio prefirió todo en una sola terminal).

**Validado:** test aislado de concurrencia (arriba). Smoke test integrado con datos reales (`Data/BTCUSD.csv`/`ETHUSD.csv`, warm start copiado de `resources/conjuntos_N/` a un directorio scratch — no se tocó el real) corriendo `_ciclo_activo` con `opcion=1`, 2 ciclos por activo, a través de `ProcessPoolExecutor` real y el `Live` real de principio a fin: sin excepciones, `_info_previa_combo`/`_resumen_combo` aparecieron una sola vez (ciclo 0), ciclo 1 mostró `Convergencia`/`cambios=... [corriendo]` sin líneas nuevas. Pendiente que Mauricio confirme visualmente el redraw en una terminal real de Windows (Mac no tiene TTY real en este entorno de desarrollo, y tampoco MT5).

## 2026-09-24 — X0: optimizador de soportes pasa de delta adaptativo a ciclos temporales

**Decisión:** eliminar `DELTA_INICIAL`/`FACTOR_DELTA` (umbral mínimo de mejora relativa, adaptativo y persistido en `{VALOR}_{N}_delta.json`/`{VALOR}_{N}_bt_delta.json`) de `nuevo_optimizador_2` y `_procesar_valor_N` — se acepta cualquier cambio que mejore la FO, sin umbral. Aplica tanto a producción en vivo como a backtesting/X5 (comparten el mismo código vía `_procesar_valor_N`). En producción en vivo, `_ciclo_activo` reemplaza la cadencia "converger y avanzar" por dos relojes de tiempo real: cada `T_UPDATE_O0` minutos (15 por defecto) se flushea el conjunto de soportes vigente a `resources/conjuntos_N/` para que X1 lo lea; cada `T_UPDATE_CICLO_X0` minutos (60 por defecto) se refresca data H1/M1, X2, X3 y se recalculan distancias antes de seguir optimizando, con warm start desde el conjunto vigente (no reinicio aleatorio). `nuevo_optimizador_2` gana un parámetro opcional `tiempo_limite_s` para correr en bloques acotados en el tiempo; en backtesting/X5 se sigue llamando sin ese límite (`tiempo_limite_s=None`), corriendo a convergencia natural o hasta los topes de seguridad `MAX_ITERS`/`MAX_CAMBIOS`, que se mantienen sin cambios. `_procesar_valor_N` se descompuso en `_preparar_valor_N` (carga + warm start + `obtener_df_extremos`, 1 vez por hora en vivo) y `_optimizar_y_guardar_valor_N` (corre el optimizador + guarda, 1 vez por bloque de 15 min en vivo o 1 vez por combo en bt/X4) para que el flujo en vivo reutilice la misma preparación entre bloques sin recalcular distancias cada 15 min — `_procesar_valor_N` queda como wrapper delgado de ambas, sin cambio de contrato para X4/X5 (que la siguen llamando igual, solo sin el parámetro `cold_start` que desapareció junto con el delta).

**Razón:** la cadencia de actualización de soportes visible para X1 pasa a ser predecible y desacoplada de cuánto tarde el optimizador en converger — antes un "ciclo" duraba lo que tomara converger (sin relación con el reloj), ahora X1 ve cambios cada `T_UPDATE_O0` minutos siempre, y los datos se refrescan cada `T_UPDATE_CICLO_X0` minutos siempre.
